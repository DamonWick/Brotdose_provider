...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnHdm = (Button) findViewById(R.id.btnHdm);
        btnHdm.setOnClickListener(this);
        btnHdm.setVisibility(View.GONE);
        pbLoader = (ProgressBar) findViewById(R.id.pbLoader);
        pbLoader.setVisibility(View.GONE);
        // Obtain the SupportMapFragment and get notified when the map is ready to be used.

        profile = FirebaseDatabase.getInstance().getReference("tiffin_provider_profile");
        uid = FirebaseAuth.getInstance().getCurrentUser().getUid();
        mapData = new HashMap<>();

        profile.child(uid).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                mapData.put("name", dataSnapshot.child("name").getValue(String.class));
                mapData.put("title", dataSnapshot.child("title").getValue(String.class));
                mapData.put("rate", dataSnapshot.child("rate").getValue(String.class));
                mapData.put("monthly rate", dataSnapshot.child("monthly rate").getValue(String.class));
                mapData.put("yearly rate", dataSnapshot.child("yearly rate").getValue(String.class));
                mapData.put("contact", dataSnapshot.child("contact").getValue(String.class));
                state = true;
            }
    ...
    private void updateMapUi(LatLng latLng) {
        btnHdm.setVisibility(View.VISIBLE);
        btnHdm.setTag(latLng);
    }
    ...
    public void onClick(View v) {
        ...
        if (btnHdm.getTag() != null) {
            LatLng selectedLoc = (LatLng) btnHdm.getTag();
            mapData.put("lat", selectedLoc.latitude);
            mapData.put("lng", selectedLoc.longitude);

            profile.child(uid).setValue(mapData, new DatabaseReference.CompletionListener() {
                @Override
                public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
                    if (databaseError == null) {
                        Toast.makeText(MapsActivity.this, "Successfull", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                }
            });
